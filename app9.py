...
# Effacer le cache lorsque cela est nécessaire
if st.button("Effacer le cache"):
    st.cache.clear()
    st.write("Cache effacé avec succès!")
...
def model_diffusion(Initial1,Initial2,epsilon, r_1, r_2, K_R, alpha, beta_1, beta_2, beta_3, beta_4, d_1, d_2, gamma_1, gamma_2, delta_1, delta_2, ra_1, ra_2, K_A, d_F, a_12, a_21, b_12, b_21, r2_1, r2_2, K2_R, alpha2, beta2_1, beta2_2, beta2_3, beta2_4, d2_1, d2_2, gamma2_1, gamma2_2, delta2_1, delta2_2, ra2_1, ra2_2, K2_A, d2_F):
    ...
    # Liste des régions uniques dans le jeu de données
    #regions = gdf['NAME_1'].unique()
    regions= gdf[gdf['NAME_1'] == 'Thies']['NAME_3'].unique()

    # Sélection des deux régions à afficher
    selected_regions = st.selectbox("Sélectionnez la première région", regions), st.selectbox("Sélectionnez la deuxième région", regions)

    # Création de la carte avec GeoPandas
    st.title("Carte des régions du Sénégal")
    fig, ax = plt.subplots()
    gdf.plot(ax=ax, color='gray')  # Rendre les autres régions en gris par défaut

    # Colorer la première région en bleu
    region1_data = gdf[gdf['NAME_1'] == 'Thies']['NAME_3'] == selected_regions[0]
    #region1_color = 'blue'
   

    # Colorer la deuxième région en rouge
    region2_data = gdf[gdf['NAME_1'] == 'Thies']['NAME_3'] == selected_regions[1]
    #region2_color = 'red'
    zone_color1 = 'blue' if most_affected_zone == 1 else 'red'
    zone_color2 = 'red' if most_affected_zone == 1 else 'blue'
      

    region1_data.plot(ax=ax, color=zone_color1)
    region2_data.plot(ax=ax, color=zone_color2)
